class OdometerGauge extends HTMLElement {
    constructor() {
        ...
        // Wait until RadialGauge is available
        const initGauge = () => {
        ...
        if (typeof RadialGauge !== 'undefined') initGauge();
        else window.addEventListener('load', initGauge);
    }

    setValue(val){ if(this._gauge)this._gauge.value=val; this.value=val; }
    setRange(minV,maxV){ if(this._gauge){ this._gauge.update({minValue:minV,maxValue:maxV}); this._gauge.draw(); } this.minValue=minV; this.maxValue=maxV; }
}
